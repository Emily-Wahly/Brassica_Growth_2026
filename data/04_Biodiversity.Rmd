...
sample_depth <- colSums(iNEXT_input_df)
iNEXT_input_df <- iNEXT_input_df[, sample_depth > 1000]
...
save(iNEXT_data, file = "/local/workdir/erw98/Brassica_Growth_2026/data/04_Biodiversity/iNEXT_data.RData")
# Inspect 
str(iNEXT_data)
typeof(iNEXT_data)
```
...
ggiNEXT(iNEXT_data, type = 1, facet.var = "Order.q") + 
  scale_color_manual(values = iNEXT_color_df$create_date) + 
  scale_fill_manual(values = iNEXT_color_df$create_date) + 
  theme_bw() + 
...
iNEXT_manual_df %>%
  ...
  ggplot(aes(x = m, y = qD, color = create_date, group = names)) + 
  geom_line() + 
  # Facet by station 
  facet_grid(Order.q~create_date, scales = "free") + 
...
shapiro.test(metadata_df$create_date)
```
...
kw_date <- 
  obs_whole_rich_df %>%
  kruskal_test(create_date ~ Experiment) 

# Look at it 
kw_date
#create_date   135       134   134 0.484 Kruskal-Wallis
# post-hoc test with Dunn's Test
# dunn_date <- 
#   obs_whole_rich_df %>%
#   dunn_test( p.adjust.method = "fdr") %>%
...
# Salinity by station 
date_ggboxplot <- 
  ggboxplot(obs_whole_rich_df, x = "date", y = "experiment",  #what to put for y instead of station
          color = "date", fill = "date", alpha = 0.3,
          outlier.shape = NA) + 
  # Add points
  ...
  stat_pvalue_manual(kw_date, hide.ns = TRUE,
                      tip.length = 0, step.increase = 0.01) +
  labs(subtitle = get_test_label(kw_date, detailed = TRUE),
       caption = get_pwc_label(kw_date)) + 
  theme_bw() + 
  theme(legend.position = "none",
